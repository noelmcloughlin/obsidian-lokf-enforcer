...
/** What the map's frontmatter records about itself. The map is written into
 *  the bundle, so it has to be a record the registrar accepts: `lokf validate`
 *  aborts a whole run on a Markdown file with no frontmatter, and OKF requires
 *  a `type`. It is therefore a `Document` with a minted `id`, and carries
 *  `generated` provenance naming this plugin as the `<producer>/<version>`
 *  actor (OKF §7) - so a curator can see at a glance that no person wrote it,
 *  and both plugins still treat it as reserved rather than as a concept to
 *  curate. */
export interface DiataxisHeader {
  /** `base_iri` + "diataxis", or null when the bundle has no usable base_iri. */
  id: string | null;
  /** OKF §7 producer actor, e.g. "lokf-registrar/0.5.0". */
  generatedBy: string;
  /** ISO-8601 timestamp of this (re)generation. */
  generatedAt: string;
}

const DIATAXIS_TITLE = "Diátaxis map";
const DIATAXIS_DESCRIPTION =
  "Map of Content that groups this bundle's concepts by Diátaxis genre. Generated by LOKF Registrar from each concept's genre field; regenerate it rather than editing it by hand.";

/** The frontmatter block for a map this plugin creates. Plain scalars only, so
 *  no YAML quoting is needed except around the timestamp. */
export function diataxisFrontmatter(header: DiataxisHeader): string {
  const lines = ["---", "type: Document"];
  if (header.id) lines.push(`id: ${header.id}`);
  lines.push(
    `title: ${DIATAXIS_TITLE}`,
    `description: ${DIATAXIS_DESCRIPTION}`,
    "genre: reference",
    "generated:",
    `  by: ${header.generatedBy}`,
    `  at: "${header.generatedAt}"`,
    "---",
    ""
  );
  return lines.join("\n");
...
function hasFrontmatter(doc: string): boolean {
  return /^---\r?\n/.test(doc);
}

/** Refresh the `generated.at` stamp on a map whose frontmatter this plugin
 *  wrote (recognised by its own `lokf-registrar/` actor, or the `lokf-enforcer/`
 *  one a build before the rename wrote - a human-authored `generated` block is
 *  never touched). Returns null when there is nothing to
 *  refresh. */
export function refreshGeneratedAt(doc: string, at: string): string | null {
  const end = doc.indexOf("\n---", 4);
  if (!hasFrontmatter(doc) || end === -1) return null;
  const fm = doc.slice(0, end);
  const re = /^(generated:\r?\n[ \t]+by:[ \t]*lokf-(?:registrar|enforcer)\/[^\r\n]*\r?\n[ \t]+at:[ \t]*)"?[^"\r\n]*"?[ \t]*$/m;
  if (!re.test(fm)) return null;
  const next = fm.replace(re, `$1"${at}"`) + doc.slice(end);
  return next === doc ? null : next;
}

/** Update a note's managed Diátaxis map block. Returns the new document, or null
 *  when it already matches. With a `header`, a map that still has no frontmatter
 *  (one an older version wrote) gains one, and a changed block also refreshes
 *  the plugin-written `generated.at` stamp - never a human-written one. */
export function applyDiataxisBlock(doc: string, entries: DiataxisEntry[], header?: DiataxisHeader): string | null {
  const spliced = spliceBlock(doc, DIATAXIS_START, DIATAXIS_END, buildDiataxisBlock(entries));
  let out = spliced ?? doc;
  let changed = spliced !== null;
  if (header) {
    if (!hasFrontmatter(out)) {
      out = diataxisFrontmatter(header) + out;
      changed = true;
    } else if (changed) {
      out = refreshGeneratedAt(out, header.generatedAt) ?? out;
    }
  }
  return changed ? out : null;
}

/** The full body of a freshly created Diátaxis map note - the record's
 *  frontmatter, a human-owned title, then the managed block, so a later re-run
 *  refreshes only the block (and the stamp). */
export function newDiataxisNote(entries: DiataxisEntry[], header: DiataxisHeader): string {
  return `${diataxisFrontmatter(header)}# ${DIATAXIS_TITLE}\n\n${buildDiataxisBlock(entries)}\n`;
}
